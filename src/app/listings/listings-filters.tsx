...
export function ListingsFilters({ filters }: { filters: Filters }) {
  ...
  const applySize = () => {
    navigate({
      min_length:    localSize.min_length    || undefined,
      max_length:    localSize.max_length    || undefined,
      min_width:     localSize.min_width     || undefined,
      max_width:     localSize.max_width     || undefined,
      min_thickness: localSize.min_thickness || undefined,
      max_thickness: localSize.max_thickness || undefined,
    })
    setSizeOpen(false)
  }
  ...
  const clearSize = () => {
    ...
    navigate({
      min_length: undefined, max_length: undefined,
      min_width: undefined,  max_width: undefined,
      min_thickness: undefined, max_thickness: undefined,
    })
    setSizeOpen(false)
  }
        ...
        <div className="relative" ref={sizeRef}>
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-sm transition-colors ${
              hasSizeFilter
                ? 'border-[#3DBE72] bg-[#E8F7EE] font-medium text-[#2A9E5A]'
                : 'border-stone-200 bg-white text-stone-700 hover:border-stone-300'
            }`}
          >
            Size
            {hasSizeFilter && <span className="h-1.5 w-1.5 rounded-full bg-[#3DBE72]" />}
            <svg
              className={`h-3.5 w-3.5 transition-transform ${sizeOpen ? 'rotate-180' : ''}`}
              fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
            >
            ...
            <div className="absolute top-full left-0 z-20 mt-2 w-72 rounded-2xl border border-stone-200 bg-white p-4 shadow-lg">
                ...
                <div key={label} className="mb-3 last:mb-0">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="Min"
                      value={localSize[minKey]}
                      onChange={e => setLocalSize(s => ({ ...s, [minKey]: e.target.value }))}
                      className="w-full rounded-lg border border-stone-200 px-2.5 py-1.5 text-sm text-stone-700 outline-none focus:border-[#3DBE72] focus:ring-1 focus:ring-[#3DBE72]/20"
                    />
                    <span className="shrink-0 text-stone-300">–</span>
                    <input
                      type="number" min="0" step="1"
                      placeholder="Max"
                      value={localSize[maxKey]}
                      onChange={e => setLocalSize(s => ({ ...s, [maxKey]: e.target.value }))}
                      className="w-full rounded-lg border border-stone-200 px-2.5 py-1.5 text-sm text-stone-700 outline-none focus:border-[#3DBE72] focus:ring-1 focus:ring-[#3DBE72]/20"
                    />
                  </div>
                </div>
              ...
              <div className="mt-4 flex gap-2">
                <button
                  type="button"
                  onClick={clearSize}
                  className="flex-1 rounded-lg border border-stone-200 py-1.5 text-sm text-stone-500 transition-colors hover:border-stone-300 hover:text-stone-700"
                >
                  Clear
                </button>
                <button
                  type="button"
                  onClick={applySize}
                  className="flex-1 rounded-lg bg-[#3DBE72] py-1.5 text-sm font-semibold text-white transition-colors hover:bg-[#2A9E5A]"
                >
                  Apply
                </button>
              </div>
